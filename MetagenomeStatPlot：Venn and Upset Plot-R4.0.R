...
Data = read.delim("genus.profile.txt", header=T, row.names= 1, sep="\t", stringsAsFactors = T, check.names = FALSE) 
# 读取 design.txt or mapping file，第三列信息随意，没有回报错
design = read.delim("design.txt", header=T, row.names= 1, sep="\t", stringsAsFactors = T, check.names = FALSE)
